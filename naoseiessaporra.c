...
int main() {
    ...
    printf("<html>");
        ...
        printf("</head>");
        printf("<body class=\"prox%d\">", jogador); // Para o CSS "saber" quem vai jogar e mudar o fundo do placar, aplica-se a classe (prox1 / prox2) � tag body e utiliza-se o seletor ("body.prox1 > a > div#placar > div#o" / "body.prox2 > a > div#placar > div#x")
            printf("<a href=\"?\">"); // Ao utilizar um link sem definir nenhum endere�o, ele reabrir� o mesmo arquivo. Ao colocar "?", ele abre o mesmo arquivo sem nenhum par�metro de URL
                printf("<div id=\"placar\">");
                    printf("<div id=\"o\">");
                        printf("<span>O</span>");
                        printf("<br>");
                        printf("<span>%d</span>", placar[0]); // Imprime o placar de O
                    printf("</div>");
                    printf("<div id=\"x\">");
                        printf("<span>X</span>");
                        printf("<br>");
                        printf("<span>%d</span>", placar[1]); // Imprime o placar de X
                    printf("</div>");
                printf("</div>");
            printf("</a>");
            printf("<div id=\"jogo\">");
                ...
                */
                for (i = 0; i < 9; i++) {
                    ...
                    printf("<a");
                    if (blocos[i] == 0) printf(" href=\"?bloco=%d&proxJogada=%d\"", i+1, jogador); // Se ningu�m tiver jogado, insere a refer�ncia de quais s�o os par�metros para efetivar a jogada naquele bloco
                    printf(">");
                    printf("<div class=\"cell%d\"></div>", blocos[i]); // Imprime 0 se ningu�m tiver jogado, 1 se a jogada for O e 2 se for X
                    printf("</a>");
